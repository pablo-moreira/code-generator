...
import br.com.atos.gc.modelo.AtributoId;
...
public class WinFrmXhtmlComponente extends Componente {
	...
	private void gerarTabEntidade(PrintWriter pw) {
		...
		for (Atributo atributo : getGc().getEntidade().getAtributos()) {
			if (!AtributoId.class.isInstance(atributo) && !AtributoOneToMany.class.isInstance(atributo)) {
				
				// Verifica se o atributo e anotado com @Version
				if (atributo.getField().getAnnotation(Version.class) != null) {
					println(pw, "\t\t\t\t\t\t");
					println(pw, "\t\t\t\t\t\t<h:outputLabel value=\"{0}:\" for=\"{1}\" />", atributo.getRotulo(), atributo.getField().getName());
		...
		for (Atributo atributo : atributos) {
			...
			String atributoLabel = atributo.getRotulo();
			...
			if (String.class.isAssignableFrom(atributo.getField().getType())) {
				...
				println(pw, "\t\t\t\t\t\t\tvar=\"{0}\" itemValue=\"#'{'{0}'}'\" itemLabel=\"#'{'{0}.{1}'}'\"", firstToLowerCase(atributo.getField().getType().getSimpleName()), atributoManyToOne.getAssociacaoAtributoDescricao());
				...
				println(pw, "\t\t\t\t\t\t\t<p:column><h:outputText value=\"#'{'{0}.{1}'}'\" /></p:column>", firstToLowerCase(atributo.getField().getType().getSimpleName()), atributoManyToOne.getAssociacaoAtributoDescricao());
